...
travel_times = [0, 1, 2, 4, 7, 8, 9, 10, 12, 14, 17, 18, 20, 21, 25, 26, 27, 28, 29, 30, 32, 34, 36, 37, 40, 41, 44, 45, 46, 48, 51]
...
# Filter stations to only those in Line 19, maintaining order
stations = []
...
for station_name in LINE_STATION_NAMES:
    if station_name in station_lookup:
        ...
        print(f"Please run insert_line_19_stations.py first to insert all stations for Line 19")
        exit(1)

print(f"Found all {len(stations)} stations for Line {LINE_NUMBER}")

# Generate routes every 25 minutes from 15.09.2025 00:00 to 15.10.2025 12:00
...
while current_start_time <= end_datetime:
    ...
    # Move to next departure (25 minutes later)
    current_start_time += timedelta(minutes=25)
